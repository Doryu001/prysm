...
# Find all non-test .go files that import logrus
# (still skip vendor; bazel/.github/etc are handled by is_excluded later)
mapfile -t FILES < <(grep -RIl --include='*.go' "$LOGRUS_IMPORT" . \
  | grep -vE '^./vendor/' \
